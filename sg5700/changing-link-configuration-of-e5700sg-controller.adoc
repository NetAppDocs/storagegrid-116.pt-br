...
xref:placing-appliance-into-maintenance-mode.adoc[Coloque o controlador E5700SG no modo de manutenção].
...
+
Se você fez alterações nas configurações de VLAN, a sub-rede do dispositivo pode ter sido alterada. Se precisar alterar os endereços IP do aparelho, siga as xref:setting-ip-configuration-sg5700.adoc[Defina a configuração IP] instruções.
