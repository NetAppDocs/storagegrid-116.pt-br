...
IMPORTANT: Você deve aplicar os hotfixes necessários depois de instalar a versão do StorageGRID. Para obter mais informações, consulte xref:../maintain/storagegrid-hotfix-procedure.adoc[procedimento de hotfix nas instruções de recuperação e manutenção].

. Leia o Contrato de Licença de Usuário final, marque a caixa de seleção e selecione *aceitar e continuar*.
. Na coluna *Instalar StorageGRID*, selecione o arquivo .tgz ou .zip para o Red Hat Enterprise Linux ou CentOS.
+
