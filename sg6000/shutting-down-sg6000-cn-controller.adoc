...
.Sobre esta tarefa
Para evitar interrupções de serviço, confirme que todos os outros nós de armazenamento estão conectados à grid antes de desligar o controlador ou desligue o controlador durante uma janela de manutenção programada, quando períodos de interrupção de serviço forem aceitáveis. Veja as informações sobre link:../monitor/monitoring-node-connection-states.html["monitorização dos estados de ligação do nó"].
