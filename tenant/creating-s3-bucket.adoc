...
+
Se você habilitar o Bloqueio de Objetos do S3 para um bucket, o versionamento do bucket será habilitado automaticamente. Você também pode xref:../s3/use-s3-object-lock-default-bucket-retention.adoc[especifique um modo de retenção padrão e um período de retenção padrão para o bucket]configurar as opções de retenção que serão aplicadas a cada objeto inserido no bucket que não especificar suas próprias configurações de retenção.
