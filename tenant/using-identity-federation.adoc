...
* Se você pretende usar TLS (Transport Layer Security) para comunicações com o servidor LDAP, o provedor de identidade deve estar usando TLS 1,2 ou 1,3. xref:../admin/supported-ciphers-for-outgoing-tls-connections.adoc[Cifras suportadas para conexões TLS de saída]Consulte .
