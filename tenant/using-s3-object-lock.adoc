...
Você também pode xref:../s3/use-s3-object-lock-default-bucket-retention.adoc[especifique um modo de retenção padrão e um período de retenção padrão para o bucket]. Essas configurações são aplicadas a cada objeto adicionado ao bucket que não especifica suas próprias configurações de retenção.
