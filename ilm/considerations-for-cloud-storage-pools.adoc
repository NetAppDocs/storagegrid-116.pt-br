...
Se você usar um servidor proxy não transparente, também deverá xref:../admin/configuring-storage-proxy-settings.adoc[Configurar um proxy de armazenamento]permitir que as mensagens sejam enviadas para endpoints externos, como um endpoint na Internet.
...
* xref:creating-cloud-storage-pool.adoc[Crie um pool de storage em nuvem]
