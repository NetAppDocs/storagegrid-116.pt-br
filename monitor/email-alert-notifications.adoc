...
Depois de configurar o servidor de e-mail SMTP, as notificações de e-mail são enviadas aos destinatários designados quando um alerta é acionado, a menos que a regra de alerta seja suprimida por um silêncio. xref:silencing-alert-notifications.adoc[Silenciar notificações de alerta]Consulte .
...
+
xref:collecting-log-files-and-system-data.adoc[Colete arquivos de log e dados do sistema]Consulte .
