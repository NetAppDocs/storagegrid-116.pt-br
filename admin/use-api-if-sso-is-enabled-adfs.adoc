...
[role="lead"]
Se você tiver xref:../admin/configuring-sso.adoc[Logon único configurado e habilitado (SSO)] e usar o ative Directory como provedor SSO, deverá emitir uma série de solicitações de API para obter um token de autenticação válido para a API de Gerenciamento de Grade ou para a API de Gerenciamento do locatário.
