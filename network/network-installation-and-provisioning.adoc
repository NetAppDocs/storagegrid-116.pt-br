...
Se pretender desativar a rede de administração ou a rede de cliente, pode remover a configuração deles durante o processo de aprovação do nó ou pode utilizar a ferramenta alterar IP após a conclusão da instalação (consulte xref:../maintain/configuring-ip-addresses.adoc[Configurar endereços IP]).
